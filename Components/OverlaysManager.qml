...
QtObject {
    ...
    // Persistir bajo demanda (p. ej. al soltar un drag de posición). No se
    // conecta offsetChanged a save() para no escribir el archivo por píxel
    // durante el arrastre.
    function persistNow() {
        root.save()
    }
    ...
    // La carga inicial se dispara aquí también (stateFile.reload()).
